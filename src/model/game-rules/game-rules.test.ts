...
describe('GameRules', () => {
    describe('isActionAllowed()', () => {
        test('should return true if action in target column is allowed', () => {
            ...
            const allowed = GameRules.isActionAllowed(boardMock, {
                columnIndex: 3,
                disc: X,
            })
    ...
    describe('getNextPlayer()', () => {
        test('should return the next player index', () => {
            const match1 = MatchFactory.build(
                GameVariation.tiny,
                PlayerType.Human
            )
            const match2 = MatchFactory.build(
                GameVariation.tiny,
                PlayerType.Human
            )
    ...
    describe('isWinningAction()', () => {
        test('should return true if the game was won', () => {
            ...
            const isWinningAction = GameRules.isWinningAction(
                boardMock,
                actionMock,
                GameVariation.tiny
            )
    ...
    describe('hasLineOfLength()', () => {
        describe('vertical line', () => {
            test('should return true if a 4 disc line was formed', () => {
                const boardMock = BoardFactory.build({ columns: 5, rows: 5 }, [
                    [X, O, X, O],
                    [X, O, O],
                    [O],
                    [X, X, X, X, X],
